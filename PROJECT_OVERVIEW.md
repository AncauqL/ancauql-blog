...
- 评论管理：`/comment`
  ...
  - 新增「类型」列区分评论与回复（回复会显示「回复 @某某」），并展示每条评论的点赞数；删除顶层评论时会提示回复也会一起删除。
- 关于我编辑：`/about/edit`
...
- `GET /comment?articleId=文章ID`：某篇文章的评论列表（公开）。返回顺序为「顶层评论按时间升序，每条顶层评论后面紧跟它的回复」；每条带 `parentId`、`replyToNickname`（回复对象昵称）、`likeCount`、`liked`（当前访客是否点过赞）。
- `POST /comment`：发表评论或回复（**需登录，任意角色**，即发即显）。请求体 `articleId / content / parentId?`（昵称取账号，服务端定名防伪造）+ 蜜罐 `website`；按 IP 每分钟限流；内容 ≤2000 字。回复会校验上级评论存在且属于同一篇文章，并统一挂到**顶层评论**下（只做两级）。
- `POST /comment/like`：点赞 / 取消点赞（**公开，未登录也能点**）。请求体 `{commentId}`；同一访客对同一条评论只能点一次，再点即取消，返回 `{likeCount, liked}`。访客标识为登录用户的 `u{id}` 或未登录的 `md5(IP+UA+盐)`。
- `GET /comment/list`：全部评论（管理员，新的在前）。
- `DELETE /comment/delete?id=评论ID`：删除评论（任意登录：管理员任意、普通用户仅本人）。**删除顶层评论会连带删除它下面的所有回复与相关点赞**。
...
- `nickname`：昵称（≤40 字）。
- `content`：评论内容（≤2000 字）。
- `parent_id`：所属顶层评论 ID（NULL=顶层评论；回复的回复也会挂到同一顶层，只有两级）。
- `reply_to_nickname`：被回复者昵称（冗余存储，对方评论删了也能正常显示「回复 @某某」）。
- `like_count`：点赞数（数据库端原子加减，不会低于 0）。
- `create_time`：发表时间。

`comment_like` 表（点赞去重）：

- `comment_id` + `visitor_key`：复合主键。`visitor_key` 对已登录用户是 `u{用户ID}`，未登录访客是 `md5(IP+UA+盐)`——同一访客只能点一次赞，再点即取消。
- 删除评论（含顶层评论连带删除的回复）时，对应点赞记录一并清理。
...
- 评论：发表评论需登录；**点赞不需要登录**（按登录用户 ID 或 `md5(IP+UA+盐)` 去重，同一访客再点即取消）。回复只支持两级（回复的回复也挂在同一顶层评论下），删除顶层评论会连带删除其回复与点赞。
